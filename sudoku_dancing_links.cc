...
//统计空的数独位置
void find_spaces(Sudoku* sudo)
{
  sudo->nspaces = 0;
  for (int cell = 0; cell < N; ++cell) {
    if (sudo->board[cell] == 0)
      sudo->spaces[sudo->nspaces++] = cell;
  }
}
//初始化数独读入
...
{
  for (int cell = 0; cell < N; ++cell) {
    sudo->board[cell] = in[cell];
    assert(0 <= sudo->board[cell] && sudo->board[cell] <= NUM);
  }
  find_spaces(sudo);
}
//是否存在可以利用的
bool available(int guess, int cell, Sudoku* sudo)
{
  for (int i = 0; i < NEIGHBOR; ++i) {
    int neighbor = sudo->neighbors[cell][i];
    if (sudo->board[neighbor] == guess) {
      return false;
    }
  }
  return true;
